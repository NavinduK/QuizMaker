						...
						<div class="form-group">
							...
							<div class="d-flex">
								<div class="col-6 d-flex">
									<input type="number" pattern="[0-9]{2}" value=00 min="0" max="24" name="hrs" required="" class="form-control" />
									<label> hr</label>
								</div>
								<div class="col-6 d-flex">
									<input type="number" pattern="[0-9]{2}" value=00 min="0" max="59" name="mins" required="" class="form-control" />
									<label> min</label>
								</div>
							</div>
						</div>
	...
	$(document).ready(function() {
			...
			$.ajax({
				...
				success: function(resp) {
					if (typeof resp != undefined) {
						...
						$('[name="hrs"]').val(Math.floor(resp.duration/60))
						$('[name="mins"] ').val(resp.duration%60)
		...
		$('#quiz-frm').submit(function(e) {
			...
			$.ajax({
				...
				data: $(this).serialize(),
				error: err => {
					console.log(err,$(this).serialize())
